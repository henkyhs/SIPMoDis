<div class="container mt-4">

  <!-- Page Header -->
  <div class="mb-3">
    <h1 class="h3 mb-0 text-gray-800">Detail Mobil</h1>
    <small class="text-muted">ID: <?= htmlspecialchars($mobil->idMobil) ?></small>
  </div>

  <div class="row">
    <div class="col-lg-8">

      <!-- Data Mobil -->
      <div class="card shadow mb-3">
        <div class="card-body">
          <div class="row">

            <div class="col-md-6 mb-3">
              <div class="text-muted small">Merk / Tipe</div>
              <div class="font-weight-bold">
                <?= htmlspecialchars($mobil->merkMobil) ?> / <?= htmlspecialchars($mobil->namaMobil) ?>
              </div>
            </div>

            <div class="col-md-6 mb-3">
              <div class="text-muted small">Plat Nomor</div>
              <div class="font-weight-bold">
                <?= htmlspecialchars($mobil->platNomor) ?>
              </div>
            </div>

            <div class="col-md-6 mb-3">
              <div class="text-muted small">Transmisi</div>
              <div class="font-weight-bold">
                <?= htmlspecialchars($mobil->transmisi) ?>
              </div>
            </div>

            <div class="col-md-6 mb-3">
              <div class="text-muted small">Kondisi Mobil</div>
              <div class="font-weight-bold">
                <span class="badge badge-info">
                  <?= status_mobil($mobil->kondisiMobil); ?>
                </span>
              </div>
            </div>

            <div class="col-md-6 mb-3">
              <div class="text-muted small">No BPKB</div>
              <div class="font-weight-bold">
                <?= htmlspecialchars($mobil->noBPKB) ?>
              </div>
            </div>

            <div class="col-md-6 mb-3">
              <div class="text-muted small">Atas Nama</div>
              <div class="font-weight-bold">
                <?= htmlspecialchars($mobil->atasNama) ?>
              </div>
            </div>

            <div class="col-md-6 mb-3">
              <div class="text-muted small">Tahun Pembuatan</div>
              <div class="font-weight-bold">
                <?= htmlspecialchars($mobil->tahun) ?>
              </div>
            </div>


          </div>
        </div>

        <!-- CTA di bawah seperti form -->
        <div class="card-footer d-flex justify-content-between align-items-center">
          <a href="<?= site_url('mobil'); ?>" class="btn btn-sm btn-outline-secondary">
            Kembali
          </a>

          <a href="<?= site_url('mobil/editForm/'.$mobil->idMobil) ?>" class="btn btn-sm btn-warning">
            Edit Data Mobil
          </a>
        </div>

      </div>

    </div>
  </div>

</div>
